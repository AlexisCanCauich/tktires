...
foreach ($lineas as $linea) {
    ...
    if( !empty($codigo) ){
        $sqlVerificarExistencia = ("SELECT codigo FROM MHankook WHERE codigo='".($codigo)."' ");
        ...
        if ( $cantidadDuplicidad == 0 ) { 
            $insertarProduct = ("INSERT INTO MHankook(codigo,marca,descripcion,precio,link) VALUES('$codigo','$marca','$descripcion','$precio','$link')");
            mysqli_query($db, $insertarProduct);
        }else {
            $updateData = ("UPDATE MHankook SET marca='" .$marca. "', descripcion='".$descripcion."', precio='".$precio."', link='".$link."'  WHERE codigo='".$codigo."' ");
